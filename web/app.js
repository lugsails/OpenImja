...
function drawBoundary(featureCollection) {
  ...
  const points = featureCollection.features.flatMap(f => f.geometry.coordinates.flat(2)).filter(p => Array.isArray(p) && typeof p[0] === "number");
...
async function load() {
  ...
  const rows = csvResponse.ok ? (await csvResponse.text()).trim().split("\n").slice(1).map(line => { const c=line.split(","); return {date:c[0],area:Number(c[1])}; }).filter(r=>r.date && Number.isFinite(r.area)) : [];
  drawChart(rows);
  ...
  const obs=latest.latest_observation, previous=rows.at(-2), yearAgo=rows.filter(r=>Math.abs(new Date(r.date)-new Date(obs.observed_at)) < 380*864e5 && Math.abs(new Date(r.date)-new Date(obs.observed_at)) > 300*864e5).at(-1);
